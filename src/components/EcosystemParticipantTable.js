...
export default function EcosystemParticipantTable() {
  ...
  useEffect(() => {
    const getData = async () => {
      const response = await getEcosystemTraceabilityTableData(
        selectedOption,
  ...
  }, [selectedOption, process.env.NEXT_PUBLIC_API_TOKEN])
  ...
  const returnTableHeaders = (selectedOptionName) => {
    if (selectedOptionName === "Regulations") {
      ...
      selectedOptionName === "Standards bodies / sustainability systems"
    ) {
  ...
  return (
    <section className="container mx-auto bg-white rounded-b-md shadow-md">
          ...
          return (
            <button
              ...
              style={{
                ...
                backgroundColor:
                  selectedOption.id === option.id ? option.bgColor : "",
              }}
            >
              {option?.name}
            </button>
      ...
      <div
        className="my-2  md:px-2 px-5 relative"
        id="cosystem-participant-table-content"
      >
        <div
        ...
        <div className="mb-5">
          {selectedOption.name ===
            "Standards bodies / sustainability systems" ||
          selectedOption.name === "Standards/Policies" ? (
            <p className="text-xs  mt-2">
              We highly recommend{" "}
              <a
                href="https://standardsmap.org/"
                className=" underline font-bold"
                target="_blank"
                rel="noreferrer"
              >
              ...
              for a more detailed list of standards bodies <br />
              and standards covering the traceability and sustainability sectors
            </p>
          ) : null}
        </div>
        ...
        <div id="ecosystem-participant-table" className="relative md:px-0 px-5">
          ...
          <div
            className="absolute  bottom-3 left-0 gap-2 cursor-pointer w-full "
          ...
          >
            ...
            >
              <img
                src="/arrow_top_icon.svg"
                alt="back to ecosystem"
