...
export default function Form({ userId, email, clientToken }) {
  ...
  return (
    <>
      ...
      <form action={formAction} className="grid gap-y-6  mt-10 " ref={formRef}>
        <input
          type="text"
          defaultValue={userId}
          name="user_id"
        ...
        />
        <div>
          <label className="uppercase font-bold text-xs mb-2 block">
            Email
          </label>
          <input
            type="email"
            ...
            name="email"
            // disabled
            className=" py-2 px-3 border rounded border-black w-full"
            required
          />
        </div>
        <div>
